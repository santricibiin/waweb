...
# ============================================================
# Install shell aliases (wa* shortcuts)
# ============================================================
step "Bonus - Install shell aliases"

ALIAS_FILE="/etc/profile.d/wa-otp-aliases.sh"

cat > "$ALIAS_FILE" <<'BASH_ALIAS_EOF'
# WA OTP Platform - convenience aliases
# Auto-generated by vps-deploy.sh. Edit langsung kalau perlu.

# Path & user
WAOTP_DIR="/opt/wa-otp"
WAOTP_USER="waotp"
WAOTP_DB_PASS_FILE="/root/.wa-otp-db-password"

# === Code management ===
alias wapull="cd \$WAOTP_DIR && sudo -u \$WAOTP_USER git fetch origin && sudo -u \$WAOTP_USER git reset --hard origin/main"
alias waupdate="sudo bash \$WAOTP_DIR/scripts/vps-update.sh"

# === Service control ===
alias warestart="sudo -u \$WAOTP_USER pm2 restart all"
alias warestart-web="sudo -u \$WAOTP_USER pm2 restart wa-otp-web"
alias warestart-worker="sudo -u \$WAOTP_USER pm2 restart wa-otp-worker"
alias wastatus="sudo -u \$WAOTP_USER pm2 status"
alias walogs="sudo -u \$WAOTP_USER pm2 logs --lines 100"
alias walogs-web="sudo -u \$WAOTP_USER pm2 logs wa-otp-web --lines 100"
alias walogs-worker="sudo -u \$WAOTP_USER pm2 logs wa-otp-worker --lines 100"

# === Database ===
alias wadb='DB_PASS=$(sudo cat $WAOTP_DB_PASS_FILE 2>/dev/null); mysql -u waotp -p"$DB_PASS" wa_otp'
alias waroot="sudo mysql wa_otp"
alias wadbpass="sudo cat \$WAOTP_DB_PASS_FILE"

# === Backup & restore ===
alias wabackup='sudo mkdir -p /root/backups && sudo mysqldump wa_otp | gzip > /root/backups/wa-otp-$(date +%Y%m%d-%H%M%S).sql.gz && echo "Backup tersimpan di /root/backups/" && ls -lh /root/backups/ | tail -5'
alias warestore="sudo bash \$WAOTP_DIR/scripts/db-restore.sh"
alias wabackups="ls -lh /root/backups/ 2>/dev/null | grep wa-otp || echo 'Belum ada backup'"

# === Files & config ===
alias waenv="sudo nano \$WAOTP_DIR/.env"
alias wacd="cd \$WAOTP_DIR"
alias waauth="ls -la \$WAOTP_DIR/wa-worker/auth-sessions/"
...
waotp-help() {
  cat <<HELP
WA OTP Platform - Available aliases:

CODE:
  wapull              Pull latest code (no rebuild)
  waupdate            Pull + install + migrate + build + restart

SERVICE:
  wastatus            PM2 status (web + worker)
  warestart           Restart both services
  warestart-web       Restart only web
  warestart-worker    Restart only worker
  walogs              Logs from both (last 100 lines)
  walogs-web          Logs from web only
  walogs-worker       Logs from worker only

DATABASE:
  wadb                Login MySQL as 'waotp' user
  waroot              Login MySQL as root (sudo)
  wadbpass            Print current DB password
  wabackup            Create gzip backup to /root/backups/
  wabackups           List backup files
  warestore           Restore from a backup file (interactive)

FILES:
  waenv               Edit .env (nano)
  wacd                cd to /opt/wa-otp
  waauth              List Baileys auth-sessions folder
...
log "Untuk aktif sekarang: source $ALIAS_FILE   (atau buka shell baru / re-login)"
